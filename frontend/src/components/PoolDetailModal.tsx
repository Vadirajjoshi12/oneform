...
/>
 ...
 {onLeavePool && (
  <button
    type="button"
    onClick={() => {
      setLeaveMemberName('your account');
      setLeavePhone('');
      setLeaveError(null);
    ...
    <div className="bg-white rounded-3xl p-6 w-full max-w-sm border border-black/10 shadow-2xl space-y-4 text-center">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1 leading-relaxed">
          Are you sure you want to cancel this order pool?
          ...
          onClick={async () => {
            const hostPhoneNum =
              ...
              localStorage.getItem(
                `oneform_host_phone_${pool.id}`
              ) ||
              '';

            if (!hostPhoneNum) {
              setHostActionError(
                'Host phone number is required. Please refresh the pool.'
              );
            ...
            try {
              ...
              setHostActionError(
                err?.message ||
                  'Failed to cancel pool'
              );
            } finally {
              setHostActionLoading(false);
            }
          }}
          className="bg-rose-600 hover:bg-rose-700 disabled:bg-slate-300 text-white text-xs font-bold py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1.5"
        >
          {hostActionLoading ? (
            <Loader2 className="w-3.5 h-3.5 animate-spin" />
          ) : null}
...
{showLeaveModal && (
  <div className="absolute inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-3xl p-6 w-full max-w-sm border border-black/10 shadow-2xl space-y-5">

      <div className="text-center">
        <div className="w-11 h-11 rounded-full bg-rose-100 text-rose-700 flex items-center justify-center mx-auto mb-3">
        ...
        <p className="text-xs text-slate-500 mt-1 leading-relaxed">
          ...
          <strong>{leaveMemberName}</strong> to confirm leaving this pool.
        </p>
      </div>
        ...
        <input
          ...
          className={`w-full bg-slate-50 border ${
            leaveError
              ? 'border-rose-400'
              : 'border-slate-200'
          } rounded-xl px-3 py-3 text-sm text-center tracking-wider focus:outline-none`}
        />

        {leaveError && (
          <div className="mt-2 text-[11px] text-rose-600 font-semibold flex items-center justify-center gap-1">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button
          ...
          className="bg-slate-100 hover:bg-slate-200 disabled:bg-slate-50 text-slate-800 text-xs font-bold py-3 rounded-xl transition-colors"
        >
          Keep Me In
        </button>
        ...
        <button
          ...
          disabled={leaveLoading}
          onClick={async () => {
            const phone = leavePhone.trim();

            if (!phone) {
              setLeaveError('Phone number is required.');
              return;
            }
            ...
            if (!onLeavePool) {
              setLeaveError('Leave Pool action is unavailable.');
              return;
            }
            ...
            try {
              await onLeavePool(pool.id, phone);
              ...
              setLeaveError(
                err?.message || 'Failed to leave pool.'
              );
            } finally {
              setLeaveLoading(false);
            }
          }}
          className="bg-rose-600 hover:bg-rose-700 disabled:bg-slate-300 text-white text-xs font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-1.5"
        >
          {leaveLoading && (
            <Loader2 className="w-3.5 h-3.5 animate-spin" />
          )}
          ...
          <span>
            {leaveLoading ? 'Leaving...' : 'Leave Pool'}
          </span>
        </button>
